...
def _print_run_details(details: AgentRunDetails) -> None:
    ...
    print(f"Segment turns (legacy max_turns): {run.max_turns}")
    print(f"Total turn limit: {_optional_number(run.total_turn_limit)}")
    print(f"Segment count: {run.segment_count}")
    print(f"Termination reason: {_optional_text(run.termination_reason)}")
