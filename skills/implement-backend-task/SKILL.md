...
2. Perform bounded workspace discovery to identify backend and shared areas.
   Do not repeatedly probe conventional directories already known to be absent.
3. Call `find_symbol` for every proposed class, function, method, endpoint,
   model, repository, service, or utility before creating it. Use the exact
   fully qualified symbol where applicable: `AuthService.logout`, not merely
   `AuthService`. Search related behavior even when names differ, and read
   plausible source matches and nearby tests. Prefer reuse or extension;
   briefly explain why existing code cannot be reused when new code is needed.
...
5. Only then call `apply_patch` with the smallest coherent change in authorized
   backend or shared paths.
6. Run the aggregate trusted check `run_check(name="backend")` after editing.
   Use individual `ruff`, `pyright`, or `pytest` checks only when requested or
...
Refuse frontend-only, cross-feature, cross-task, and unassigned-task work.
